...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    for c in cnts:
        ...
        if (M["m00"] > 30 and (30 < cntArea < 50)):
            ...
            mengru2dict['key'] = cX
            mengru2dict['contourArea'].add(cntArea)
            # sorted(mengru2dict['contourArea'])
            print(mengru2dict)
            ...
            # while (len(lst2['xCoordinates'])) == 4:
            #     if tlblX != lst2['xCoordinates'][0] and tlblX != lst2['xCoordinates'][2] :
            #         lst2['xCoordinates'][0] = int(tlblX)
            #         lst2['xCoordinates'][1] = int(trbrX)
            #         lst2['yCoordinates'][0] = tlblY
            #         lst2['Difference'] = lst2['yCoordinates'][0] - lst2['yCoordinates'][1]
            #         lst2['xCoordinates'].sort()
            #         lst2['yCoordinates'].sort()
            #         if lst2['xCoordinates'][0] == 0 and lst2['xCoordinates'][1] == 0:
            #             break
            #         elif lst2['yCoordinates'][1] == 0:
            #             break
            #     else:
            #         continue
            #
            #     boxes['A']['xCoordinates'] = (lst2['xCoordinates'][0], lst2['xCoordinates'][1])
            #     boxes['B']['xCoordinates'] = (lst2['xCoordinates'][2], lst2['xCoordinates'][3])
            #     boxes['A']['yCoordinates'] = lst2['yCoordinates'][0]
            #     boxes['B']['yCoordinates'] = lst2['yCoordinates'][1]
            #     boxes['C']['Difference'] = pix * abs(lst2['Difference'])
            #     # print(boxes)
            #
            #     # sorted(lst.extend([tlblX, trbrX]))
            #     if len(lst2['xCoordinates']) == 4:
            #         # print(lst2['xCoordinates'])
            #         break
            #
            # # Draw box based on those four corners
            # cv2.drawContours(cropped, [box.astype("int")], -1, (0, 255, 0), 1)
            #
            # # Draw line in the middle of each rectangular box
            # cv2.line(cropped, (int(tlblX), int(tlblY)), (int(trbrX), int(trbrY)), (0, 0, 255), 1)
            #
            # # Calculate gradient of the rectangles
            # gradientAvg = (gradient(tr, br) + gradient(tl, bl))/ 2
            # cv2.putText(cropped, "Box A: " + str(boxes['A']['xCoordinates']), (5, 120), cv2.QT_FONT_NORMAL, 0.5, (255, 255, 255), thickness=1)
            # cv2.putText(cropped, "Box B: " + str(boxes['B']['xCoordinates']), (550, 120), cv2.QT_FONT_NORMAL, 0.5, (255, 255, 255), thickness=1)
            # cv2.putText(cropped, "Difference is " + str(boxes['C']['Difference']) + " um", (250, 250), cv2.QT_FONT_NORMAL, 0.5, (0, 0, 0), thickness=1)

    # Display the video
    # cv2.imshow("SimpleLive_Python_uEye_OpenCV", flip_frame)
